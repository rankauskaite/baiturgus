...
type Category = typeof NAVBOARD_CATEGORIES[number]
        ...
        {isOpen ? (
            <div className={cn('absolute inset-x-0 top-full text-sm text-muted-foreground', {
                'animate-in fade-in-10 slide-in-from-top-5': !isAnyOpen,}
                    ...
                    <div className='absolute inset-0 top-1/2 bg-pink-50 shadow' aria-hidden='true' />
                                ...
                                <div className='col-span-4 col-start-1 grid grid-cols-3 gap-x-8'>
                                    {category.featured.map((item) => (
                                        <div key={item.name} className='group relative text-base sm:text-sm'>
                                            <div className='relative aspect-video overflow-hidden rounded-lg bg-pink-100 group-hover:opacity-75'>
